...
const foodList = ['짜장면', '짬뽕', '피자'];
foodList.forEach(food => console.log(`${food} 사주세요~`));
...
const userABC = userList.filter(user => user.account==='abc1234');
console.log(userABC);
...
const firstHooby = userList.map(u => u.hobbys[0]);
...
console.log(`================================================`);
userList // [{5},{5},{5},{5}]
  .filter(use => use.address ==='서울') // -> [{5},{5},{5}]
  .map(use => {
    return{
      name: use.userName,
      secondHobby: use.hobbys[1],
    };t
  })
